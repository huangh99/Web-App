...
<script>
import { getCommentsAPI, postCommentAPI, addCommentLikeAPI, cancelCommentLikeAPI } from '@/api/articleAPI'
...
export default {
  ...
  methods: {
    ...
    async getCommentList() {
      const { data: res } = await getCommentsAPI(this.articleId)
      if (res.status === 0) {
    ...
    async postComment() {
      const { data: res } = await postCommentAPI(this.articleId, this.commentContent)
      if (res.status === 0) {
